...
Both answers must match. The API is reached through the gateway, the model through
...
API_HOST = "inference.fraudstream.localhost"
...
async def main() -> int:
    ...
    async with httpx.AsyncClient(**client_options(API_HOST), timeout=60) as api:
        served = (await api.post("/v1/predict", json=payment)).json()
    async with httpx.AsyncClient(timeout=60) as model:
        direct = (
            await model.post(MODEL, json={"instances": [inputs]}, headers={"Host": MODEL_HOST})
        ).json()["predictions"][0]
